...
public class CliConfiguration {
  /**
   * Singleton Mordant terminal. Auto-detects ANSI capability: emits full ANSI on
   * TTY,
   * plain text when piped. Shared across all commands so cursor state is
   * consistent.
   *
   * <p>
   * Mordant's Terminal has no no-arg Java constructor (Kotlin default params
   * without
   * {@code @JvmOverloads}). We use ServiceLoader to discover the
   * platform-specific
   * {@link TerminalInterface} — exactly what Kotlin does when
   * {@code terminalInterface = null}.
   */
  @Bean
  Terminal terminal() {
